...
public class BraitenbergVehikel extends Observable {
	...
	public BraitenbergVehikel(String name, BVBewegung bewegung, Vektor2 position, Vektor2 orientierung) {
		this.name = name;
	 ...
	 */
	public void bewege() {
		double umdrehungenLinks = bewegung.berechneMotorAnsteuerungLinks(sensorWert[Richtung.LINKS.ordinal()],
				sensorWert[Richtung.RECHTS.ordinal()]) * getMaxUmdrehungenProSek();
		double umdrehungenRechts = bewegung.berechneMotorAnsteuerungRechts(sensorWert[Richtung.LINKS.ordinal()],
				sensorWert[Richtung.RECHTS.ordinal()]) * getMaxUmdrehungenProSek();
		double motorBewegungLinks = umdrehungenLinks * Math.PI * getRadRadius() * 2.0;
		double motorBewegungRechts = umdrehungenRechts * Math.PI * getRadRadius() * 2.0;
	...
	protected void bewege(double streckeLinks, double streckeRechts) {
		if (Math.abs(streckeLinks - streckeRechts) < 1e-5) {
			...
			if (streckeLinks <= streckeRechts) {
				double x = (-seitenlaenge * streckeLinks) / (streckeLinks - streckeRechts);
				winkelBogenmass = streckeRechts / (x + seitenlaenge);
				rotationszentrum = position.subtrahiere(orientierung.skaliere(seitenlaenge / 2.0))
						.addiere(getLinksVektor().skaliere(seitenlaenge / 2.0 + x));
			} else {
				double x = (seitenlaenge * streckeRechts) / (streckeLinks - streckeRechts);
	...
	public String toString() {
		return "Braitenberg-Vehikel " + getName() + " (p: " + position + ", o: " + orientierung + ")";
	}
}
